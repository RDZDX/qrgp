...
void handle_sysevt(VMINT message, VMINT param) {
    ...
    switch (message) {
        ...
        case VM_MSG_PAINT:
            ...
            if (strlen(tekstas) > 0 && trigeris7 == 0) {set_single_qrcode(tekstas);}
            break;
...
void handle_keyevt(VMINT event, VMINT keycode) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_OK && sms_gen_mode == VM_FALSE) {
    ...
    if (event == VM_KEY_EVENT_UP && keycode == VM_KEY_OK && sms_gen_mode == VM_TRUE) {
       trigeris9 = 1;
       mre_read_sms();
    }
...
void set_single_qrcode(char* str) {
    ...
    lenght1 = strlen(str);
    if (!str || lenght1 == 0) {
       //if (trigeris2 == 0) {mre_draw_white_rectangle();}
...
void mre_read_sms() {
    ...
    //vm_sms_read_msg(vm_sms_get_msg_id(VM_SMS_BOX_INBOX, 0), 0, message_data, mre_sms_read_callback_function, NULL);
        ...
        if (callback_data->cause == VM_SMS_CAUSE_NO_ERROR) {
            switch (callback_data->action) {
                ...
                case VM_SMS_ACTION_READ:
                    //sms_nmb_display = sms_k - (sms_n - 1);
                    sms_nmb_display = (sms_k + 1) - sms_n;
                    content_buffer_size = message_data->content_buff_size;
                    content_buffer = message_data->content_buff;
                    //vm_ucs2_to_ascii (tekstas, content_buffer_size, (VMWSTR)content_buffer);
                    vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, content_buffer, tekstas, content_buffer_size);
                    //vm_ascii_to_ucs2(file_pathw1, 8 , "SMS"); // sms_k/sms_k
                    //sprintf(sms_num, "SMS # %d of %d", sms_n, sms_k);
                    //sprintf(sms_num, "SMS # %d of %d", sms_k, sms_k);
