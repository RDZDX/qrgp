...
VMUINT nread;
//VMCHAR tekstas[535];
VMCHAR tekstas[1002]; //(500+1)*2=1002 | (459+1)*2=920???
VMINT qVersion = 9;
VMINT trigeris = 1;
...
void mre_read_sms() {
    ...
    message_data = vm_malloc(sizeof(vm_sms_msg_data_t)); // 1sms size 160 ascii | 70 utf8 ?
        ...
        if (callback_data->cause == VM_SMS_CAUSE_NO_ERROR) {
            switch (callback_data->action) {
                ...
                case VM_SMS_ACTION_READ:
                    ...
                    vm_chset_convert(VM_CHSET_UTF16LE, VM_CHSET_UTF8, (VMSTR)message_data->content_buff, tekstas, vm_wstrlen((VMWSTR)message_data->content_buff) + 1);
                    //vm_chset_convert(VM_CHSET_UCS2, VM_CHSET_UTF8, (VMSTR)message_data->content_buff, tekstas, vm_wstrlen((VMWSTR)message_data->content_buff) + 1);
...
int screenshot_grayscale_8bit(void) {
    ...
    VMFILE f_read = vm_file_open((VMWSTR)s, MODE_CREATE_ALWAYS_WRITE, 1);
    ...
    write_uint32_le(&fileHeader[2], fileSize);

    unsigned char infoHeader[40] = {0};
    infoHeader[0] = 40;
    write_uint32_le(&infoHeader[4], w);
    write_uint32_le(&infoHeader[8], h);
    infoHeader[12] = 1;
    infoHeader[14] = 8;
    write_uint32_le(&infoHeader[20], pixelDataSize);
    write_uint32_le(&infoHeader[32], 256);
    ...
    vm_file_write(f_read, fileHeader, 14, &p);
    vm_file_write(f_read, infoHeader, 40, &p);
    vm_file_write(f_read, palette, 1024, &p);

    unsigned char* line_buf = vm_malloc(rowSize);
    if (!line_buf) {
        vm_file_close(f_read);
        return -2;
    }
    ...
    for (i = h - 1; i >= 0; i--) {
        ...
        vm_file_write(f_read, line_buf, rowSize, &p);
    }
